...
func withProviderConfigRef(kind, name string) func(*v1alpha1.Feature) {
	...
	cases := map[string]struct {
		...
		"ProviderConfigResolvesClient": {
			args: args{
				...
				cr: newFeature(withProviderConfigRef("ProviderConfig", connectTestPCName)),
			},
			want: want{err: nil},
		},
		"ClusterProviderConfigResolvesClient": {
			args: args{
				...
				cr: newFeature(withProviderConfigRef("ClusterProviderConfig", connectTestCPCName)),
			},
			want: want{err: nil},
		},
		"UnsupportedProviderConfigKind": {
			args: args{
				...
				newClientFn: func(creds []byte) (FeatureClient, error) { return &fakeClient{}, nil },
				cr:          newFeature(withProviderConfigRef("BogusConfig", "whatever")),
			},
