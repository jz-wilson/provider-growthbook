...
func withProviderConfigRef(kind, name string) func(*v1alpha1.Environment) {
	return func(cr *v1alpha1.Environment) {
	...
	cases := map[string]struct {
		...
		"ProviderConfigResolvesClient": {
			args: args{
				...
				cr: environment(withProviderConfigRef("ProviderConfig", connectTestPCName)),
			},
			want: want{err: nil},
		},
		"ClusterProviderConfigResolvesClient": {
			args: args{
				...
				cr: environment(withProviderConfigRef("ClusterProviderConfig", connectTestCPCName)),
			},
			want: want{err: nil},
		},
		"UnsupportedProviderConfigKind": {
			args: args{
				...
				newClientFn: func(creds []byte) (EnvironmentClient, error) { return &fakeClient{}, nil },
				cr:          environment(withProviderConfigRef("BogusConfig", "whatever")),
			},
